// --------------------
// CCL Script.js - Guardado directo en Supabase
// --------------------
...
const phaseDescriptions = {
  ...
  // ---------- Supabase client ----------
  const supabaseUrl = 'https://iezdsseiuskiszarpneh.supabase.co';
  const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImllemRzc2VpdXNraXN6YXJwbmVoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTgyNDg1MDgsImV4cCI6MjA3MzgyNDUwOH0.9xDWRDr6wKckkK5VtgrCi6cPCAv0nyTfT_MLEMmkY0s';
  const supabase = supabase.createClient(supabaseUrl, supabaseKey);
  
  // ---------- Guardar licencia directamente ----------
  async function saveLicenseDirect() {
    const projectTitle = document.getElementById('projectTitle').value;
    const yourName = document.getElementById('yourName').value;
    const phases = ['research','ideation','design','coding','prototyping','documentation','management','reflection'];
    const levels = {};
    ...
    let licenseCode = "CCL ";
    phases.forEach((phase,index)=>{
      if(levels[phase] > 0) licenseCode += `${phaseAbbreviations[phase]}${levels[phase]} `;
    });
    ...
    if(error) console.error('Error saving license:', error);
    else console.log('License saved ✅', data);
  }
